...
function sendInMission(spaceship: { name: string, pilot: string, crewLimit: number, crew: string[], inMission: boolean }) {
  if (spaceship.inMission) {
    alert(`A ${spaceship.name} não pode ser enviada. Nave já está em missão!`);
  } else if (spaceship.crew.length < Math.floor(spaceship.crewLimit / 3)) {
    alert(`Nave não pode ser enviada. Tripulação insuficiente!`);
  } else {
